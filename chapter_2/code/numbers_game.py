...
class Hypothesis:
    ...
    def multiples(multipliers):
        ...
        return [Hypothesis((lambda m: lambda x: x % m == 0)(multiplier), f'mult of {multiplier}') for multiplier in
                multipliers]
    ...
    def powers(bases):
        ...
        for base in bases:
            consistent_numbers = [np.power(base, power) for power in
                                  range(1, 1 + int(np.floor(math.log(number_range[-1], base))))]
...
class Graphing:
    ...
    def plot_post_membership_marginals(data, hypotheses, posterior, marginals, filename):
        ...
        extension_points = [[x, i] for i in range(top_n) for x in
                            top_10_ranked_hypotheses_posterior[i][0].get_extension()]
        ...
        ax_extensions.scatter([point[0] for point in extension_points], [point[1] for point in extension_points],
        ...
        ax_posterior.set_title(f"$p(h|{','.join([str(x) for x in data])})$", y=-0.12)
...
Graphing.plot_prior_lik_post(data_all, hypotheses, prior, likelihood_all, posterior_all,
                             'numbersGamePriorLikPostAll.pdf')
Graphing.plot_post_membership_marginals(data_16, hypotheses, posterior_16, post_marginals_16,
                                        'numbersGamePostMembership16.pdf')
